...
namespace Capstone_Backend.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(AppDbContext context, IUserRepository userRepository,IRegistrationRepository registrationRepository)
        {
            _context = context;
        ...
        {
            // Check if the username is null or if it has less than 5 letters
            if (string.IsNullOrEmpty(username) || username.Length < 5)
            {
                return new Response<Object>
                {
                    StatusCode = 400,
                    StatusMessage = "Username must be at least 5 characters long.",
                    Data = null
                };
            }

            // Proceed to edit the profile
        ...
        [HttpGet("get-apartment-details")]
        public async Task<ActionResult<AppartmentGetDTO>> GetAppartment(int id)
        {
            var host = Request.Host.ToString(); // Get the host from the request
            var appartment = await _userRepository.GetAppartmentById(id, host);
        ...
        public async Task<ActionResult<Response<List<ResponsesGetDTO>>>> GetUserResponses(string userId)
        {
            try
            {
                var host = Request.Host.ToString(); // Get the host from the request
                var responses = await _userRepository.GetUserResponses(userId, host);
        ...
        [HttpGet("reserved-apartments-count")]
        public async Task<IActionResult> GetNumberOfReservedApartmentsAsync()
        {
            int count = await _userRepository.GetNumberOfReservedApartmentsAsync();
            return Ok(count);
        }

        [HttpGet("rent-apartments-count")]
        public async Task<IActionResult> GetNumberOfApartmentsForRentAsync()
        {
            int count = await _userRepository.GetNumberOfPropertiesForRentAsync();
            return Ok(count);
        }
        [HttpGet("sale-apartments-count")]
        public async Task<IActionResult> GetNumberOfApartmentsForSaleAsync()
        {
            int count = await _userRepository.GetNumberOfPropertiesForSaleAsync();
            return Ok(count);
        }
        [HttpGet("all-apartments-count")]
        public async Task<IActionResult> GetNumberOfUploadedApartmentsAsync()
        {
            int count = await _userRepository.GetNumberOfUploadedApartmentsAsync();
            return Ok(count);
        }
